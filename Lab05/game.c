...
void initGame() {
    ...
    //TODO 4.2: Load the kirby image's colors
	DMANow(3, kirbyPal, PALETTE, 256);
...
void initBalls() {
	...
	for (int i = 0; i < BALLCOUNT; i++) {
		...
		// Make the first ball a katamari
		if(i == 0){
			balls[i].isKirby = 1;
			balls[i].height = 16;
			balls[i].width = 16;
...
void updateBall(BALL* b) {
	...
	if (b->active) {
		...
		for (int i = 0; i < BULLETCOUNT; i++) {
			if (bullets[i].active && !b->isKirby && collision(b->col, b->row, b->width, b->height,
				bullets[i].col, bullets[i].row, bullets[i].width, bullets[i].height)) {

				// Put ball back in the pool
				bullets[i].active = 0;
				
				// Turn balls into katamari
				b->isKirby = 1;
				b->height = 16;
				b->width = 16;
...
void drawBall(BALL* b) {
	...
	// UNCOMMENT 4.0
	 if(b->active) {
	 	if (b->isKirby)
	 		drawImage4(b->col, b->row, b->width, b->height, kirbyBitmap);
	 	else
